...
namespace System.Net.Sockets
{
    internal static partial class SocketPal
    {
        internal static class Unix
        {
            ...
            {
                return Interop.Unix.Sys.PlatformSupportsDualModeIPv4PacketInfo();
            }
#endif
            ...
            public static SocketError GetSocketErrorForErrorCode(Interop.Unix.Error errorCode)
            {
                return SocketErrorPal.Unix.GetSocketErrorForNativeError(errorCode);
            }
...
#if !MONO
            private static unsafe IPPacketInformation GetIPPacketInformation(Interop.Unix.Sys.MessageHeader* messageHeader, bool isIPv4, bool isIPv6)
            {
                if (!isIPv4 && !isIPv6)
                {
                ...
                Interop.Unix.Sys.IPPacketInformation nativePacketInfo;
                if (!Interop.Unix.Sys.TryGetIPPacketInformation(messageHeader, isIPv4, &nativePacketInfo))
                {
                    return default(IPPacketInformation);
                }
            ...
            {
                return SafeCloseSocket.Unix.CreateSocket(addressFamily, socketType, protocolType, out socket);
            }

#if !MONO
            private static unsafe int Receive(SafeCloseSocket socket, SocketFlags flags, byte[] buffer, int offset, int count, byte[] socketAddress, ref int socketAddressLen, out SocketFlags receivedFlags, out Interop.Unix.Error errno)
            {
                ...
                fixed (byte* b = buffer)
                {
                    var iov = new Interop.Unix.Sys.IOVector {
                        Base = (b == null) ? null : &b[offset],
                        Count = (UIntPtr)count
                    };

                    var messageHeader = new Interop.Unix.Sys.MessageHeader {
                    ...
                    errno = Interop.Unix.Sys.ReceiveMessage(socket, &messageHeader, flags, &received);
                    receivedFlags = messageHeader.Flags;
                    sockAddrLen = messageHeader.SocketAddressLen;
                }

                if (errno != Interop.Unix.Error.SUCCESS)
                {
                    return -1;
                }
...
#if !MONO
            private static unsafe int Send(SafeCloseSocket socket, SocketFlags flags, byte[] buffer, ref int offset, ref int count, byte[] socketAddress, int socketAddressLen, out Interop.Unix.Error errno)
            {
                int sent;
                ...
                fixed (byte* b = buffer)
                {
                    ...
                    long bytesSent;
                    errno = Interop.Unix.Sys.SendMessage(socket, &messageHeader, flags, &bytesSent);
...
#if !MONO
            private static unsafe int Send(SafeCloseSocket socket, SocketFlags flags, IList<ArraySegment<byte>> buffers, ref int bufferIndex, ref int offset, byte[] socketAddress, int socketAddressLen, out Interop.Unix.Error errno)
            {
                ...
                var iovecs = new Interop.Unix.Sys.IOVector[maxBuffers];
                ...
                try
                {
                    ...
                    // Make the call
                    fixed (byte* sockAddr = socketAddress)
                    fixed (Interop.Unix.Sys.IOVector* iov = iovecs)
                    {
                        var messageHeader = new Interop.Unix.Sys.MessageHeader {
                            SocketAddress = sockAddr,
                            SocketAddressLen = sockAddrLen,
...
#if !MONO
            private static unsafe long SendFile(SafeCloseSocket socket, SafeFileHandle fileHandle, ref long offset, ref long count, out Interop.Unix.Error errno)
            {
                long bytesSent; 
                errno = Interop.Unix.Sys.SendFile(socket, fileHandle, offset, count, out bytesSent);
...
#if !MONO
            private static unsafe int Receive(SafeCloseSocket socket, SocketFlags flags, IList<ArraySegment<byte>> buffers, byte[] socketAddress, ref int socketAddressLen, out SocketFlags receivedFlags, out Interop.Unix.Error errno)
            {
                int available;
                errno = Interop.Unix.Sys.GetBytesAvailable(socket, &available);
                if (errno != Interop.Unix.Error.SUCCESS)
                {
                    receivedFlags = 0;
                    return -1;
...
#if !MONO
            private static unsafe int ReceiveMessageFrom(SafeCloseSocket socket, SocketFlags flags, byte[] buffer, int offset, int count, byte[] socketAddress, ref int socketAddressLen, bool isIPv4, bool isIPv6, out SocketFlags receivedFlags, out IPPacketInformation ipPacketInformation, out Interop.Unix.Error errno)
            {
                Debug.Assert(socketAddress != null, "Expected non-null socketAddress");

                int cmsgBufferLen = Interop.Unix.Sys.GetControlMessageBufferSize(isIPv4, isIPv6);
                var cmsgBuffer = stackalloc byte[cmsgBufferLen];

                int sockAddrLen = socketAddressLen;

                Interop.Unix.Sys.MessageHeader messageHeader;
                ...
                fixed (byte* b = buffer)
                {
                    ...
                    messageHeader = new Interop.Unix.Sys.MessageHeader {
                        SocketAddress = sockAddr,
                        SocketAddressLen = sockAddrLen,
                        IOVectors = &iov,
                        IOVectorCount = 1,
            ...
            public static unsafe bool TryCompleteAccept(SafeCloseSocket socket, byte[] socketAddress, ref int socketAddressLen, out IntPtr acceptedFd, out SocketError errorCode)
            {
                ...
                fixed (byte* rawSocketAddress = socketAddress)
                {
                    try
                    {
                        errno = Interop.Unix.Sys.Accept(socket, rawSocketAddress, &sockAddrLen, &fd);
                    }
                ...
                acceptedFd = (IntPtr)(-1);
                if (errno != Interop.Unix.Error.EAGAIN && errno != Interop.Unix.Error.EWOULDBLOCK)
                {
            ...
            public static unsafe bool TryStartConnect(SafeCloseSocket socket, byte[] socketAddress, int socketAddressLen, out SocketError errorCode)
            {
                ...
                fixed (byte* rawSocketAddress = socketAddress)
                {
                    err = Interop.Unix.Sys.Connect(socket, rawSocketAddress, socketAddressLen);
                }
                ...
                if (err != Interop.Unix.Error.EINPROGRESS)
                {
            ...
            {
                Interop.Unix.Error socketError;
                Interop.Unix.Error err;
                try
                {
                    err = Interop.Unix.Sys.GetSocketErrorOption(socket, &socketError);
                }
                catch (ObjectDisposedException)
                {
                    // The socket was closed, or is closing.
                ...
                if (err != Interop.Unix.Error.SUCCESS)
                {
                    Debug.Assert(err == Interop.Unix.Error.EBADF, $"Unexpected err: {err}");
                    errorCode = SocketError.SocketError;
                    return true;
                }

                if (socketError == Interop.Unix.Error.SUCCESS)
                {
                    errorCode = SocketError.Success;
                    return true;
                }
                else if (socketError == Interop.Unix.Error.EINPROGRESS)
                {
                    errorCode = SocketError.Success;
                    return false;
                }
            ...
            public static unsafe SocketError GetSockName(SafeCloseSocket handle, byte[] buffer, ref int nameLen)
            {
                ...
                {
                    err = Interop.Unix.Sys.GetSockName(handle, rawBuffer, &addrLen);
                }

                nameLen = addrLen;
                return err == Interop.Unix.Error.SUCCESS ? SocketError.Success : GetSocketErrorForErrorCode(err);
            }

            public static unsafe SocketError GetAvailable(SafeCloseSocket handle, out int available)
            {
                int value = 0;
                Interop.Unix.Error err = Interop.Unix.Sys.GetBytesAvailable(handle, &value);
                available = value;
            ...
            public static unsafe SocketError GetPeerName(SafeCloseSocket handle, byte[] buffer, ref int nameLen)
            {
                ...
                fixed (byte* rawBuffer = buffer)
                {
                    err = Interop.Unix.Sys.GetPeerName(handle, rawBuffer, &addrLen);
                }
            ...
            {
#if MONO_FEATURE_NO_BSD_SOCKETS
                throw new PlatformNotSupportedException ();
#else
                Interop.Unix.Error err;
                fixed (byte* rawBuffer = buffer)
                {
                    err = Interop.Unix.Sys.Bind(handle, rawBuffer, nameLen);
                }
            ...
            {
                Interop.Unix.Error err = Interop.Unix.Sys.Listen(handle, backlog);
                return err == Interop.Unix.Error.SUCCESS ? SocketError.Success : GetSocketErrorForErrorCode(err);
            }

            public static SocketError Accept(SafeCloseSocket handle, byte[] buffer, ref int nameLen, out SafeCloseSocket socket)
            {
                return SafeCloseSocket.Unix.Accept(handle, buffer, ref nameLen, out socket);
            }
            ...
            public static unsafe SocketError SetSockOpt(SafeCloseSocket handle, SocketOptionLevel optionLevel, SocketOptionName optionName, int optionValue)
            {
                ...
                if (optionLevel == SocketOptionLevel.Socket)
                {
                    if (optionName == SocketOptionName.ReceiveTimeout)
                    {
                        ...
                        err = Interop.Unix.Sys.SetReceiveTimeout(handle, optionValue);
                        return err == Interop.Unix.Error.SUCCESS ? SocketError.Success : GetSocketErrorForErrorCode(err);
                    }
                    else if (optionName == SocketOptionName.SendTimeout)
                    {
                        handle.SendTimeout = optionValue == 0 ? -1 : optionValue;
                        err = Interop.Unix.Sys.SetSendTimeout(handle, optionValue);
                        return err == Interop.Unix.Error.SUCCESS ? SocketError.Success : GetSocketErrorForErrorCode(err);
                    }
                }
                else if (optionLevel == SocketOptionLevel.IP)
                {
                    if (optionName == SocketOptionName.MulticastInterface)
                    {
                        ...
                        {
                            var opt = new Interop.Unix.Sys.IPv4MulticastOption {
                                MulticastAddress = 0,
                                LocalAddress = 0,
                                InterfaceIndex = interfaceIndex
                            };

                            err = Interop.Unix.Sys.SetIPv4MulticastOption(handle, Interop.Unix.Sys.MulticastOption.MULTICAST_IF, &opt);
                            return err == Interop.Unix.Error.SUCCESS ? SocketError.Success : GetSocketErrorForErrorCode(err);
                        }
                    }
                }

                err = Interop.Unix.Sys.SetSockOpt(handle, optionLevel, optionName, (byte*)&optionValue, sizeof(int));
                return err == Interop.Unix.Error.SUCCESS ? SocketError.Success : GetSocketErrorForErrorCode(err);
            }
#endif
            ...
            public static unsafe SocketError SetSockOpt(SafeCloseSocket handle, SocketOptionLevel optionLevel, SocketOptionName optionName, byte[] optionValue)
            {
                ...
                {
                    err = Interop.Unix.Sys.SetSockOpt(handle, optionLevel, optionName, null, 0);
                }
                else
                {
                    fixed (byte* pinnedValue = optionValue)
                    {
                        err = Interop.Unix.Sys.SetSockOpt(handle, optionLevel, optionName, pinnedValue, optionValue.Length);
                    }
                }
            ...
            public static unsafe SocketError SetMulticastOption(SafeCloseSocket handle, SocketOptionName optionName, MulticastOption optionValue)
            {
                ...
                Interop.Unix.Sys.MulticastOption optName = optionName == SocketOptionName.AddMembership ?
                    Interop.Unix.Sys.MulticastOption.MULTICAST_ADD :
                    Interop.Unix.Sys.MulticastOption.MULTICAST_DROP;
                ...
                Interop.Unix.Error err = Interop.Unix.Sys.SetIPv4MulticastOption(handle, optName, &opt);
                return err == Interop.Unix.Error.SUCCESS ? SocketError.Success : GetSocketErrorForErrorCode(err);
            }
#endif
            ...
            public static unsafe SocketError SetIPv6MulticastOption(SafeCloseSocket handle, SocketOptionName optionName, IPv6MulticastOption optionValue)
            {
                ...
                var opt = new Interop.Unix.Sys.IPv6MulticastOption {
                    Address = optionValue.Group.GetNativeIPAddress(),
                    InterfaceIndex = (int)optionValue.InterfaceIndex
                };

                Interop.Unix.Error err = Interop.Unix.Sys.SetIPv6MulticastOption(handle, optName, &opt);
                return err == Interop.Unix.Error.SUCCESS ? SocketError.Success : GetSocketErrorForErrorCode(err);
            }
#endif
            ...
            {
                var opt = new Interop.Unix.Sys.LingerOption {
                    OnOff = optionValue.Enabled ? 1 : 0,
                    Seconds = optionValue.LingerTime
                };

                Interop.Unix.Error err = Interop.Unix.Sys.SetLingerOption(handle, &opt);
                return err == Interop.Unix.Error.SUCCESS ? SocketError.Success : GetSocketErrorForErrorCode(err);
            }
#endif
            ...
            public static unsafe SocketError GetSockOpt(SafeCloseSocket handle, SocketOptionLevel optionLevel, SocketOptionName optionName, out int optionValue)
            {
                ...
                {
                    Interop.Unix.Error socketError = default(Interop.Unix.Error);
                    Interop.Unix.Error getErrorError = Interop.Unix.Sys.GetSocketErrorOption(handle, &socketError);
                    optionValue = (int)GetSocketErrorForErrorCode(socketError);
                    return getErrorError == Interop.Unix.Error.SUCCESS ? SocketError.Success : GetSocketErrorForErrorCode(getErrorError);
                }

                int value = 0;
                int optLen = sizeof(int);
                Interop.Unix.Error err = Interop.Unix.Sys.GetSockOpt(handle, optionLevel, optionName, (byte*)&value, &optLen);
            ...
            public static unsafe SocketError GetSockOpt(SafeCloseSocket handle, SocketOptionLevel optionLevel, SocketOptionName optionName, byte[] optionValue, ref int optionLength)
            {
                ...
                {
                    optLen = 0;
                    err = Interop.Unix.Sys.GetSockOpt(handle, optionLevel, optionName, null, &optLen);
                }
                ...
                else
                {
                    fixed (byte* pinnedValue = optionValue)
                    {
                        err = Interop.Unix.Sys.GetSockOpt(handle, optionLevel, optionName, pinnedValue, &optLen);
                    }
                }
            ...
            public static unsafe SocketError GetMulticastOption(SafeCloseSocket handle, SocketOptionName optionName, out MulticastOption optionValue)
            {
                ...
                Interop.Unix.Sys.IPv4MulticastOption opt;
                Interop.Unix.Error err = Interop.Unix.Sys.GetIPv4MulticastOption(handle, optName, &opt);
                if (err != Interop.Unix.Error.SUCCESS)
                {
            ...
            public static unsafe SocketError GetIPv6MulticastOption(SafeCloseSocket handle, SocketOptionName optionName, out IPv6MulticastOption optionValue)
            {
                ...
                Interop.Unix.Sys.IPv6MulticastOption opt;
                Interop.Unix.Error err = Interop.Unix.Sys.GetIPv6MulticastOption(handle, optName, &opt);
                if (err != Interop.Unix.Error.SUCCESS)
                {
            ...
            {
                var opt = new Interop.Unix.Sys.LingerOption();
                Interop.Unix.Error err = Interop.Unix.Sys.GetLingerOption(handle, &opt);
                if (err != Interop.Unix.Error.SUCCESS)
                {
            ...
            {
                Interop.Unix.Sys.PollEvents inEvent = Interop.Unix.Sys.PollEvents.POLLNONE;
                switch (mode)
                {
                    case SelectMode.SelectRead: inEvent = Interop.Unix.Sys.PollEvents.POLLIN; break;
                    case SelectMode.SelectWrite: inEvent = Interop.Unix.Sys.PollEvents.POLLOUT; break;
                    case SelectMode.SelectError: inEvent = Interop.Unix.Sys.PollEvents.POLLPRI; break;
                }

                int milliseconds = microseconds == -1 ? -1 : microseconds / 1000;

                Interop.Unix.Sys.PollEvents outEvents;
                Interop.Unix.Error err = Interop.Unix.Sys.Poll(handle, inEvent, milliseconds, out outEvents);
                if (err != Interop.Unix.Error.SUCCESS)
                {
                    status = false;
                    return GetSocketErrorForErrorCode(err);
                ...
                switch (mode)
                {
                    case SelectMode.SelectRead: status = (outEvents & (Interop.Unix.Sys.PollEvents.POLLIN | Interop.Unix.Sys.PollEvents.POLLHUP)) != 0; break;
                    case SelectMode.SelectWrite: status = (outEvents & Interop.Unix.Sys.PollEvents.POLLOUT) != 0; break;
                    case SelectMode.SelectError: status = (outEvents & (Interop.Unix.Sys.PollEvents.POLLERR | Interop.Unix.Sys.PollEvents.POLLPRI)) != 0; break;
            ...
            public static unsafe SocketError Select(IList checkRead, IList checkWrite, IList checkError, int microseconds)
            {
                ...
                {
                    Interop.Unix.Sys.PollEvent* eventsOnStack = stackalloc Interop.Unix.Sys.PollEvent[count];
                ...
                {
                    var eventsOnHeap = new Interop.Unix.Sys.PollEvent[count];
                    fixed (Interop.Unix.Sys.PollEvent* eventsOnHeapPtr = eventsOnHeap)
                    {
                        return SelectViaPoll(
                            checkRead, checkReadInitialCount,
                            checkWrite, checkWriteInitialCount,
            ...
            private static unsafe SocketError SelectViaPoll(
                ...
                Interop.Unix.Sys.PollEvent* events, int eventsLength,
                int microseconds)
            {
                ...
                int offset = 0;
                AddToPollArray(events, eventsLength, checkRead, ref offset, Interop.Unix.Sys.PollEvents.POLLIN | Interop.Unix.Sys.PollEvents.POLLHUP);
                AddToPollArray(events, eventsLength, checkWrite, ref offset, Interop.Unix.Sys.PollEvents.POLLOUT);
                AddToPollArray(events, eventsLength, checkError, ref offset, Interop.Unix.Sys.PollEvents.POLLPRI);
                Debug.Assert(offset == eventsLength, $"Invalid adds. offset={offset}, eventsLength={eventsLength}.");

                // Do the poll
                uint triggered = 0;
                int milliseconds = microseconds == -1 ? -1 : microseconds / 1000;
                Interop.Unix.Error err = Interop.Unix.Sys.Poll(events, (uint)eventsLength, milliseconds, &triggered);
                if (err != Interop.Unix.Error.SUCCESS)
                {
                    return GetSocketErrorForErrorCode(err);
                }
                ...
                {
                    FilterPollList(checkRead, events, checkReadInitialCount - 1, Interop.Unix.Sys.PollEvents.POLLIN | Interop.Unix.Sys.PollEvents.POLLHUP);
                    FilterPollList(checkWrite, events, checkWriteInitialCount + checkReadInitialCount - 1, Interop.Unix.Sys.PollEvents.POLLOUT);
                    FilterPollList(checkError, events, checkErrorInitialCount + checkWriteInitialCount + checkReadInitialCount - 1, Interop.Unix.Sys.PollEvents.POLLERR | Interop.Unix.Sys.PollEvents.POLLPRI);
                }
                return SocketError.Success;
            }
#endif
...
#if !MONO
            private static unsafe void AddToPollArray(Interop.Unix.Sys.PollEvent* arr, int arrLength, IList socketList, ref int arrOffset, Interop.Unix.Sys.PollEvents events)
            {
                if (socketList == null)
                    return;
                ...
                for (int i = 0; i < listCount; i++)
                {
                    ...
                    arr[arrOffset++] = new Interop.Unix.Sys.PollEvent { Events = events, FileDescriptor = fd };
                }
            }
#endif

#if !MONO
            private static unsafe void FilterPollList(IList socketList, Interop.Unix.Sys.PollEvent* arr, int arrEndOffset, Interop.Unix.Sys.PollEvents desiredEvents)
            {
                if (socketList == null)
                    return;
            ...
            {
                Interop.Unix.Error err = Interop.Unix.Sys.Shutdown(handle, how);
                if (err == Interop.Unix.Error.SUCCESS)
                {
                    return SocketError.Success;
                }
                ...
                if (err == Interop.Unix.Error.ENOTCONN && (isConnected || isDisconnected))
                {
                    return SocketError.Success;
                }
...
#if !MONO
        public const int ProtocolInformationSize = Unix.ProtocolInformationSize;

        public const bool SupportsMultipleConnectAttempts = Unix.SupportsMultipleConnectAttempts;
